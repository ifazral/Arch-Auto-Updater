...
# SC2034 Fix: LAST_RUN_EPOCH was unused. Commented out to save resources.
# LAST_RUN_EPOCH=$(cat "$LOG_FILE")
NEWS_ACK_EPOCH=$(cat "$NEWS_ACK_LOG")
NEWS_RETRIES=$(cat "$RETRY_COUNT_FILE")

# SC1090 Fix: Tell ShellCheck to ignore dynamic sourcing
...
if [ -n "$METERED_STATUS" ]; then
    if [ "$IGNORE_METERED" = "1" ]; then
        ...
        # SC2086 Fix: Added double quotes to $YAD_EXIT
...
if [ "$SKIP_UPDATES" -eq 0 ]; then
    ...
    sudo topgrade -y 2>&1 | tee -a /tmp/arch_updater_topgrade.log > /dev/null
    ...
    # SC2086 Fix: Added double quotes to $TOPGRADE_EXIT
